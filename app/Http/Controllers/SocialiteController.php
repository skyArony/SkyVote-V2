...
class SocialiteController extends Controller
{
    ...
    public function QQ(Request $request) {
        if(!Redis::EXISTS('voters:'.$request->activity_key)) return $this->setResponse(null, 400, -4005);
        $request->session()->put('activity_key', $request->activity_key);
        ...
        if(!Voter::find($user['id'])) {
            ...
            switch ($request->driver) {
                case 'qq':
                    ...
                    else Redis::sadd("voters:".session('activity_key'), $user['id']);
                    break;
            }
        }

        $request->session()->pull('activity_key');
