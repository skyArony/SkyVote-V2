...
{{--我自己加的--}}
@include('vendor.voyager.candidates.my-browse-add-session')
{{--我自己加的--}}
...
@section('page_header')
    <div class="container-fluid">
        <h1 class="page-title">
            ...
            {{--我自己加的--}}
            - @include('vendor.voyager.candidates.my-browse-ac-name')
            {{--我自己加的--}}
        </h1>
        ...
        @endcan
        {{--我自己加的--}}
        @include('vendor.voyager.candidates.my-browse-back')
        {{--我自己加的--}}
                            ...
                            <table id="dataTable" class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    @endcan
                                        ...
                                        {{--非link类型时，去掉linkCover的显示--}}
                                        @php
                                            $count = 0;
                                            foreach($dataType->browseRows as $row) {
                                                if($row->field == 'textContent' && session('type') == 'img') unset(($dataType->browseRows)[$count]);
                                                if($row->field == 'imageContent' && (session('type') == 'video' || session('type') == 'audio')) unset(($dataType->browseRows)[$count]);
                                                $count++;
                                            }
                                        @endphp
                                        {{--我自己加的thead--}}
                                ...
                                if(session('id')) {
                                    ...
                                    foreach ($dataTypeContent as $data)
                                        if($data->belong_ac_id == session('id')) $array[] = $data;
                                ...
                                @foreach($dataTypeContent as $data)
                                    <tr>
                                        @can('delete',app($dataType->model_name))
                                            ...
                                            {{--我自己加的--}}
                                            @include('vendor.voyager.candidates.my-browse-show-type')
                                            {{--我自己加的--}}
                                        @endcan
                                        @foreach($dataType->browseRows as $row)
                                            <td>
