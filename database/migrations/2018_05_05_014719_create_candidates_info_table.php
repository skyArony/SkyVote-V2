...
class CreateCandidatesInfoTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('candidates_info', function (Blueprint $table) {
            ...
            $table->string('img_url', 512)->comment('图片链接');
            $table->string('video_url', 512)->comment('视频链接');
            $table->string('audio_url', 512)->comment('音频链接');
            $table->json('link_info')->comment('外链的链接和封面，linkurl + linkcoverurl');
            $table->timestamps();
